;relatively stable version of TSR
;needs optimization
...
_int2fh_remove:
    ...
    mov ax, 252fh
    ;ds:dx
    mov ds, [cs:prev_handler]
    mov dx, [cs:prev_handler+2]
    int 21h
...
__help: ;obvious
    ...
    ;check just this interrupt handled by dos
    mov ax, 0xF000
    int 2fh
...
__install: ;set TSR
    ...
    int 21h
    mov word [prev_handler+2], bx ;?
    mov word [prev_handler], es ;?
    ...
    mov dx, (_size - _entry_point + 10fh) / 16 + 100h
    ;mov dx, 0x60
    mov ax, 3100h
    int 21h
...
_check_tsr: ;check if TSR already loaded ;? what is the return?
    mov ax, 0c800h ;resident ID, 00 - installation check
    int 2fh ;multiplex interrupt
